...
def build_insights(latest, history, load, prev_load, age):
    ...
    if not items:
        # Хороший чек-ин — повод предложить измеримую техническую цель, а не автоматически повышать нагрузку.
        action = ('На следующем занятии поставь цель чуть сложнее обычной: выбери один удар и сравни точность двух коротких серий. После запиши результат и самочувствие.'
                  if latest['energy'] >= 4 and latest['fatigue'] <= 2 and latest['stress'] <= 2
                  else 'На следующем занятии выбери один удар и поработай над его точностью в двух коротких сериях. После запиши результат и самочувствие.')
