...
function about(){return header('О RallyGuard','Рабочий прототип для Overclock Hackathon · BioTech')+`<div class="page-grid"><section class="panel"><h2>Игрок в центре</h2><p>Короткий опрос превращает субъективные показатели в понятный обзор. Вместо одинакового урока для любой проблемы игрок видит, что изменилось и какой следующий шаг относится именно к его ответам.</p><h3>Что можно проверить</h3><ol class="plan-steps"><li>Заполнить опрос сна, энергии, усталости, стресса и дискомфорта.</li><li>Посмотреть отдельные наблюдения и действия по своим ответам.</li><li>Записать тренировку и увидеть динамику нагрузки.</li><li>Сравнить три вымышленных сценария в настройках.</li></ol><button class="btn lime" data-page="checkin">Перейти к опросу →</button></section><section class="panel"><h2>Границы прототипа</h2><p>Все стартовые профили вымышлены. Пороги индекса — демонстрационные правила; клиническая точность и снижение травматизма не исследованы.</p><p>Приложение не определяет причину боли, не диагностирует выгорание и не выдаёт медицинский допуск к нагрузке. Предложения по физической части требуют обсуждения с тренером или специалистом.</p><p class="score-explainer">Разбор сейчас выполняют прозрачные правила Python. Генеративный AI можно добавить для объяснения уже рассчитанных сигналов после настройки API и проверки безопасности данных.</p></section></div>`;}

function onboarding(){navRender();$('#main').innerHTML=`<div class="onboarding"><section class="hero"><div class="hero-content"><span class="eyebrow">ТЕННИС В ТВОЁМ ТЕМПЕ</span><h2>Пойми своё<br>состояние</h2><p>Отметь сон, энергию и нагрузку. Увидь, что изменилось и на что обратить внимание сегодня.</p><button class="btn lime" data-action="start">Посмотреть демо →</button></div></section><section class="panel"><h2>От ответов к ясному действию</h2><div class="intro-list"><div>${icon('sun')}<strong>Заполни опрос</strong><p>Сон, энергия, стресс, усталость и дискомфорт.</p></div><div>${icon('chart')}<strong>Увидь причины</strong><p>Личная динамика и отдельное объяснение каждого сигнала.</p></div><div>${icon('team')}<strong>Обсуди следующий шаг</strong><p>Подсказки игроку и обзор для тренера.</p></div></div><div class="notice">Это демо хакатона с вымышленными игроками. Не вводи реальные сведения о здоровье. Индекс не является медицинской оценкой.</div></section></div>`;}

async function render(){
  const revision=++routeVersion; page=location.hash.slice(1)||'today';navRender();
  if(!current)return onboarding();
  const views={today:overview,checkin:checkinPage,progress,coach,settings,about};
